...
Step 2 — Which path? (A/B/C/D)
   A. URL extraction       → load extractors/apify-pipeline.md
...
The skill should always ask the gate question the same way:
...
If the user said "yes" but didn't indicate a path, ask Step 2 verbatim:

> "How would you like to provide the design?
>
> **A.** Extract from a website URL (recommended for real brands you have access to).
...
> "Running [Path X] on [target]. This will take ~90 seconds for Path A, ~5 seconds for Path B/C, ~2 minutes for Path D. Reply 'cancel' if you want to choose differently."
...
|---|---|---|
| "Use the same brand as my last project" | Path C (load existing DESIGN.md from another folder) or Step 0 (use the BRAND.md already in this folder) | Check the current project folder first. If empty, ask "which project's DESIGN.md should I copy?" |
